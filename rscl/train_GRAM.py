# GRAM training script for LIBERO/GR00T.
#
# This leaves train.py untouched. Run with:
#   python -m rscl.train_GRAM --dataset_path <libero suite dirs...>
...
class ArgsConfig:
    ...
    output_dir: str = "./outputs/libero_gram"
    ...
    w_depth: float = 1.0   # depth distance weight
    w_vel: float = 1.0     # state velocity distance weight
...
class GRAMTrainer(DualBrainTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        lam = self.gram_config.lambda_init * 0.5 * (1 + math.cos(math.pi * step / self.total_steps))
        ...
        logs = {
            ...
            "gram_pos_neg_gap": outputs.get("gram_pos_neg_gap", torch.tensor(0.0)).item(),
            "rscl_target_entropy": outputs.get("rscl_target_entropy", torch.tensor(0.0)).item(),
        }
        self.log(logs)
...
class DepthAugmentedDataset(torch.utils.data.Dataset):
    # dataset wrapper that adds extra modalities (depth; velocity is derived from state in the action head)
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        run_name=f"rscl-gram-{config.data_config.split(':')[-1] if ':' in config.data_config else config.data_config}-s{config.seed}",
        seed=config.seed,
